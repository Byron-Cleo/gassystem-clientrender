...
import { listMonthly13kgRevenue } from '../actions/analyticsActions';
import Loader from '../components/Loader';
import Message from '../components/Message';

const Gas13KgMonthlyRevenueListScreen = ({ history }) => {
  //method to fire the action
  const dispatch = useDispatch();

  //Select the loaded customers' list from central state
  const gas13KgMonthRevenueList = useSelector(
    (state) => state.gas13KgMonthRevenueList
  );
  const { loading, error, month13KgRevenues } = gas13KgMonthRevenueList;
  // console.log(topTenCustomers);

  useEffect(() => {
    dispatch(listMonthly13kgRevenue());
  }, [dispatch]);
  ...
  return (
    <>
      ...
      <Row className="align-items-center">
        <Col>
          <h1>13KG Gas Monthly Revenue</h1>
        </Col>
      </Row>
      <>
        {loading && <Loader />}
        ...
        <Table striped bordered hover responsive className="table-sm">
          ...
          <tbody>
            {month13KgRevenues.map((gas6KgMonthlyRevenue) => (
...
export default Gas13KgMonthlyRevenueListScreen;
